...
public class CustomerDaoImpl implements CustomerDao {
	...
	public Customer getCustomerByemailId(String emailId) {
		if (httpsession.getAttribute("customerInfo") != null ) {
			return (Customer)httpsession.getAttribute("customerInfo");
		} else {
			Session session = sessionFactory.openSession();
	...
	public Authorities getAuthoritiesByemailId(String emailId) {
		if (httpsession.getAttribute("customerAuth") != null ) {
			return (Authorities)httpsession.getAttribute("customerAuth");
		} else {
			Session session = sessionFactory.openSession();
			Query query = session.createQuery("from Authorities where emailId=?");
			query.setString(0, emailId);
			Authorities authorities = (Authorities) query.uniqueResult();
			httpsession.setAttribute("customerAuth", authorities);
			return authorities;
		}
	}
